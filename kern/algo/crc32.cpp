...
namespace synos { namespace algo { namespace crc32 {
static const unsigned int lookup_table[] = { 0x00000000, 0x04c11db7, 0x09823b6e,
...
uint_32
crc32(const stdk::string target, uint_32 init)
{
	uint_32 crc = init;
	for (uint_32 i = 0; i < target.length(); ++i)
		crc =
		    (crc << 8) ^ lookup_table[((crc >> 24) ^ *target[i]) & 255];
	return crc;
}
}}};
